...
func accountAction(ctx *cli.Context) error {
	...
	if keygen := ctx.Bool(GenerateFlag.Name); keygen {
		...
		var file string
		file, err = keystore.GenerateKeypair(keytype, basedir, password)
		if err != nil {
	...
	if keyimport := ctx.String(ImportFlag.Name); keyimport != "" {
		...
		// import keypair
		_, err = keystore.ImportKeypair(keyimport, basedir)
		if err != nil {
	...
	if keylist := ctx.Bool(ListFlag.Name); keylist {
		_, err = utils.KeystoreFilepaths(basedir)
		if err != nil {
...
func unlockKeystore(ks *keystore.Keystore, basedir string, unlock string, password string) error {
	...
	if password != "" {
		...
		err := keystore.UnlockKeys(ks, basedir, unlock, password)
		if err != nil {
