...
func TestGlobalConfigFromFlags(t *testing.T) {
	...
	testcases := []struct {
		...
		{
			"Test gossamer --base-dir",
			[]string{"config", "base-dir"},
			[]interface{}{testCfgFile.Name(), "test_base-dir"},
			dot.GlobalConfig{
				Name:    testCfg.Global.Name,
				ID:      testCfg.Global.ID,
				BaseDir: "test_base-dir",
			},
		},
		{
			"Test gossamer --roles",
...
func TestUpdateConfigFromGenesisData(t *testing.T) {
	...
	db, err := database.NewBadgerDB(cfg.Global.BaseDir)
	require.Nil(t, err)
