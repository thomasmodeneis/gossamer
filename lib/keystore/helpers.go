...
// it to basedir/keystore/[public key].key in json format encrypted using the
// specified password and returns the resulting filepath of the new key
func GenerateKeypair(keytype string, basedir string, password []byte) (string, error) {
	if keytype == "" {
	...
	keyPath, err := utils.KeystoreDir(basedir)
	if err != nil {
