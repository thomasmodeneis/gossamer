...
// DefaultBaseDir the default value for Config.BaseDir
const DefaultBaseDir = "~/.gossamer/gssmr"
...
type Config struct {
	// BaseDir the data directory for the node
	BaseDir string
	...
	if c.RandSeed == 0 {
		...
		if key == nil {
			log.Info(
				...
				"RandSeed", c.RandSeed,
				"KeyFile", path.Join(c.BaseDir, DefaultKeyFile),
			)

			// generate key
			key, err = generateKey(c.RandSeed, c.BaseDir)
			if err != nil {
				return err
			}
		}
